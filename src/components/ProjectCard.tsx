    ...
    <article className="card">
      <div
        className={`grid h-full items-center text-base-content ${
          image ? "sm:grid-cols-[57.75%_minmax(0,1fr)]" : ""
        }`}
      >
        {image ? <ProjectImage src={image} title={title} /> : null}

        <div className="min-w-0 h-full w-full flex flex-col items-start justify-center gap-y-3.75 py-5.25 px-8.75 text-left">
            ...
            <div className="flex w-full flex-col items-start gap-2.75 ">
              <div className="flex w-full flex-col items-start gap-0.5 ">
                <h3 className="card-title flex w-full font-bold items-end gap-2.25 leading-tight tracking-[-0.0675em] text-base-content/85 md:text-3xl">
                  <span className="font-serif-display inline-flex min-w-0 flex-wrap items-baseline justify-start gap-x-2">
                    <span>{title}</span>
                  </span>
                  {titleAside && (
              ...
              {description && (
                <p className="text-sm leading-tight tracking-[-0.0325em] pl-0.25 text-base-content/70 md:text-lg">
                  {description}
                </p>
              )}
            </div>
