...
const navLinks = [
  { label: "Projets personnels", sectionId: "personal-projects-section" },
  { label: "Réalisations freelance", sectionId: "client-projects-section" },
  {
    label: "Activités complémentaires",
    sectionId: "complementary-activities-section",
  },
];
...
export function TopBar() {
  ...
  async function handleSectionLinkClick(
    ...
    window.requestAnimationFrame(() => {
      ...
      const scrollOffset = topBarHeight + 24;
      const top =
          ...
          <div className="flex flex-col gap-1 border-t border-base-content/10 py-3">
            {navLinks.map((link) => (
              <a
                ...
                key={link.sectionId}
                onClick={(event) =>
                  handleSectionLinkClick(event, link.sectionId)
                }
              >
                {link.label}
              </a>
